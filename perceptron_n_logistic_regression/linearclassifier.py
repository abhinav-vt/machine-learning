...
def linear_predict(data, model):
    ...
    return np.argmax(np.transpose(model['weights']).dot(data), axis=0)
...
def perceptron_update(data, model, params, label):
    ...
    alpha = params['alpha']
    prediction = linear_predict(data=data, model=model)

    model['weights'][:, label] += alpha * data
    model['weights'][:, prediction] -= alpha * data
...
def log_reg_train(data, labels, model, check_gradient=False):
    ...
    def log_reg_nll(new_weights):
        ...
        theta_dot_x = np.transpose(new_weights).dot(data)

        nll = logsumexp(theta_dot_x, dim=0)
        for c in range(num_classes):
            nll -= np.where(labels == c, np.transpose(new_weights[:, c]).dot(data), 0)
        nll = np.sum(nll)
        ...
        # p_ij matrix
        p_ij = np.exp(theta_dot_x - logsumexp(theta_dot_x, dim=0))

        for c in range(num_classes):
            # minus 1 for rows with given label
            p_ij[c] -= np.where(labels == c, 1, 0)
            # sum across n data points
            gradient[:, c] = np.sum(data*p_ij[c], axis=1)
